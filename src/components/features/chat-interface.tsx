    ...
    try {
      if (typeof data === "string" && data.length > 200) {
        ...
        if (Array.isArray(clonedData) && clonedData.length > 5) {
          formattedData = [
            ...clonedData.slice(0, 5),
            `... 及${clonedData.length - 5}项更多`,
          ];
        }
        // 处理长字符串属性
          ...
          Object.keys(clonedData).forEach((key) => {
            if (
              typeof clonedData[key] === "string" &&
              clonedData[key].length > 100
            ) {
              clonedData[key] =
                clonedData[key].substring(0, 100) + "... [截断]";
            }
          });
...
const simulateStreamingOutput = (
  ...
  const outputNextChunk = () => {
    if (currentIndex < content.length) {
      ...
      // 尝试直接DOM更新
      try {
        const aiMsgElem = document.querySelector(
...
export default function ChatInterface() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const res = await fetch("/openapi/v1/chat/completions", {
        method: "POST",
                ...
                switch (eventType) {
                  case "answer":
                    ...
                    if (payload.choices && payload.choices[0]?.delta?.content) {
                      ...
                      try {
                        ...
                        if (aiMsgElem) {
                          const contentElem =
                            aiMsgElem.querySelector(".message-content");
                          if (contentElem) {
                    ...
                    if (
                      ...
                      Object.keys(payload.choices[0]?.delta).length === 0
                    ) {
                      forceLog(
                        "接收到delta但无content字段:",
                        payload.choices[0]?.delta
                  ...
                  case "flowResponses":
                    ...
                    if (Array.isArray(payload)) {
                      ...
                      // 查找AI对话节点的回复
                      const aiChatNode = payload.find(
                        (node: any) =>
                          node.moduleType === "chatNode" &&
                          node.moduleName?.includes("AI") &&
                          (node.answer ||
                            node.responseData?.choices?.[0]?.message?.content)
                      );

                      if (aiChatNode) {
                        const answer =
                          aiChatNode.answer ||
                          aiChatNode.responseData?.choices?.[0]?.message
                            ?.content;
                        if (answer) {
                      ...
                      // 查找知识库引用源
                      const searchNode = payload.find(
                        (node: any) =>
                          node.moduleType === "datasetSearchNode" &&
                          node.quoteList
                      );
                      ...
                      if (
                        ...
                        searchNode.quoteList.length > 0
                      ) {
                        forceLog(
                          "从flowResponses提取到引用来源:",
                          searchNode.quoteList
                        );
                        console.log(
                          "【调试】完整知识库引用数据:",
                          JSON.stringify(searchNode.quoteList, null, 2)
                        );

                        // 检查引用中是否有文件URL
                        searchNode.quoteList.forEach(
                          (quote: any, index: number) => {
                            console.log(`【调试】引用 ${index + 1} 详情:`, {
                              sourceName: quote.sourceName,
                              q:
                                quote.q?.substring(0, 50) +
                                (quote.q?.length > 50 ? "..." : ""),
                              url: quote.url || "未提供",
                        ...
                        const references = searchNode.quoteList.map(
                          (quote: any) => {
